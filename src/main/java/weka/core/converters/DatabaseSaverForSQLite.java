...
//    }
     ...
     * @throws IOException throws IOException
     */
    public void writeBatchExcep() throws Exception {
...
//                for (int j = 0; j < inst.numAttributes(); j++) {
//                    if (inst.attribute(j).isNumeric()) inst.setValue(j, Math.round(inst.value(j) * 327)); //16bits
//                    if (inst.attribute(j).isNumeric()) inst.setValue(j, Math.round(inst.value(j) * 83880)); //24bits
//                }
