 ...
 * @param weighted
 */
case class KNNBatchb(k: Int, distance_name: String, pattsForDistanceCache: Seq[Pattern], weighted: Boolean = false) extends BatchWekaLearner {
   override val toString = k + "NNb" + (if (weighted) " weighted " else " (") + distance_name + s")"
